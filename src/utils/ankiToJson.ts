...
export default async function ankiToJson(input: File) {
	const zip = new JSZip()
	const SQL = await initSqlJs({
		locateFile: (file) => `https://cdnjs.cloudflare.com/ajax/libs/sql.js/1.8.0/sql-wasm.wasm`
	})

	zip.loadAsync(input, { base64: false, checkCRC32: true }).then((zip) => {

		const db = new SQL.Database(zip.files['collection.anki2'])
		db.exec('SELECT id, flds, sfld FROM notes').forEach((note) => {
			console.log(note)
		})
